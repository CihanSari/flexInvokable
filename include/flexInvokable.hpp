...
struct flexInvokable<R(Args...)> {
	...
	template<class F,
		std::enable_if_t < !std::is_same<std::decay_t<F>, flexInvokable>{}, int > = 0,
...
#endif
	, R>{} || std::is_same<R, void>{},
		int
		> = 0
	>

		flexInvokable(F&& f) :
		flexInvokable(flexInvokableInternal::emplace_as<std::decay_t<F>>{}, std::forward<F>(f))
	{}
